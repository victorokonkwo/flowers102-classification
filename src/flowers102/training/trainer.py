...
from flowers102.utils.visualization import plot_learning_curve, plot_confusion_matrix
from flowers102.models.resnet import load_checkpoint
...
class Trainer:
    ...
    def _train_one_epoch(self) -> Tuple[float, float]:
        ...
        Returns:
            Tuple of (average losss, accuracy)
        """
        self.model.train()
        running_loss = 0.0
    ...
    def test(self) -> Tuple[float, float]:
        ...
        if best_model_path.exists():
            ...
            self.model._load_state_dict(checkpoint['model_state_dict'])
            logger.info(f"Loaded best model with validation accuracy: {checkpoint['accuracy']:.4f}")
    ...
    def _save_checkpoint(self, epoch: int, accuracy: float, is_best: bool = False):
        ...
        torch.save(checkpoint, self.config.checkpoint_dir / f"checkpoint_epoch_{epoch+1}.pth")
        ...
        for epoch in range(self.config.num_epochs):
            ...
            if epoch == 10 and self.config.freeze_backbone:
                ...
                self.optimizer = optim.SGD(
                    ...
                    lr=self.config.learning_rate / 10,  # Lower learning rate for finetuning
                ...
                self.scheduler = ReduceLROnPlateau(
                    self.optimizer, model='min', factor=0.1, patience=5, verbose=True
                )
